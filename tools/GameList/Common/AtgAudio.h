...
// NOTE: Use this class only for XMA v1 and older XMA2 files. For new XAudio2 compataible
//       Xma files use WaveFile above.
//--------------------------------------------------------------------------------------
class XMAFile
{
    HANDLE m_hFile;            // File handle
...
public:
    ...
    XMAWAVEFORMAT* GetFormat() const
    {
        return m_pFormat;
    }
    XMA2WAVEFORMAT* GetXma2Format() const
    {
        return m_pFormat2;
    }
    DWORD* GetSeekTable() const
    {
        return m_pSeekTable;
    }
    WORD    GetNumStreams() const;
    BYTE    GetStreamChannels( WORD wStreamIndex ) const;
    DWORD   GetStreamSampleRate( WORD wStreamIndex ) const;
