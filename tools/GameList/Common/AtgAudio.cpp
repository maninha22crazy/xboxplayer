...
HRESULT CreateSourceVoiceInit( const XMAFile* pXmaFile, XAUDIOSOURCEVOICEINIT* pSourceVoiceInit )
{
    ...
    pSourceVoiceInit->Format.NumStreams = ( XAUDIOXMASTREAMCOUNT )pXmaFile->GetNumStreams();

    assert( pXmaFile->GetNumStreams() <= XAUDIOXMASTREAMCOUNT_MAX );
    for( WORD i = 0; i < pXmaFile->GetNumStreams(); ++i )
    {
        pSourceVoiceInit->Format.Stream[ i ].SampleRate = pXmaFile->GetStreamSampleRate( i );
        pSourceVoiceInit->Format.Stream[ i ].ChannelCount = ( XAUDIOCHANNEL )pXmaFile->GetStreamChannels( i );
    }
...
//--------------------------------------------------------------------------------------
HRESULT SampleToBlockOffset( DWORD dwSampleIndex, const DWORD* pdwSeekTable, DWORD nEntries, DWORD* out_pBlockIndex,
                             DWORD* out_pOffset )
{
...
//--------------------------------------------------------------------------------------
// Name: GetNumStreams()
// Desc: Gets the number of interleaved XMA streams
//-------------------------------------------------------------------------------------
WORD XMAFile::GetNumStreams() const
{
    WORD wNumStreams = 0;
    if( m_fXMA2 )
    {
        wNumStreams = m_pFormat2->NumStreams;
    }
    else
    {
        wNumStreams = m_pFormat->NumStreams;
    }
    return wNumStreams;
}
...
//--------------------------------------------------------------------------------------
// Name: GetStreamChannels
// Desc: Returns the number of channels for the stream identified by the passed index.
//-------------------------------------------------------------------------------------
BYTE XMAFile::GetStreamChannels( WORD wStreamIndex ) const
{
    assert( wStreamIndex < GetNumStreams() );
    BYTE bChannels = 0;
    if( m_fXMA2 )
    {
        bChannels = m_pFormat2->Streams[wStreamIndex].Channels;
    }
    else
    {
        bChannels = m_pFormat->XmaStreams[wStreamIndex].Channels;
    }
    return bChannels;
}
...
//--------------------------------------------------------------------------------------
// Name: GetStreamSampleRate()
// Desc: Returns the sample rate for a specific XMA stream identified by the passed index.
//       All XMA2 interleaved streams have the same sample rate.
//-------------------------------------------------------------------------------------
DWORD XMAFile::GetStreamSampleRate( WORD wStreamIndex ) const
{
    assert( wStreamIndex < GetNumStreams() );
    DWORD dwSampleRate = 0;
    // All XMA2 streams have the same sample rate
    if( m_fXMA2 )
    {
        dwSampleRate = m_pFormat2->SampleRate;
    }
    else
    {
        dwSampleRate = m_pFormat->XmaStreams[wStreamIndex].SampleRate;
    }
    return dwSampleRate;
}
