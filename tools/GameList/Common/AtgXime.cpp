...
{
    if ( m_JPUserWord >= MAX_JP_USERWORD )
        return S_FALSE;
    ...
    if ( pReading[ 0 ] && pDisplay[ 0 ] && pImePos[ 0 ] )
    {
        ...
        if ( pos )
        {
            if ( XIMERegisterUserWord( pReading, pDisplay, pos, &s_imeOverLapped ) == ERROR_IO_INCOMPLETE )
            {
                ...
                DWORD result = XGetOverlappedResult( &s_imeOverLapped, NULL, TRUE );
                ...
                else if ( result == ERROR_DATABASE_FULL )
                {
